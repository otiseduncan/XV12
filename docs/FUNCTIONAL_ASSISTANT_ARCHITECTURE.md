...
The protected conversation path remains one model streaming call for ordinary chat. `AssistantOrchestrator` adds a maximum four-round native function loop only when Qwen selects a registry-supplied capability. It does not parse tool-shaped text or rewrite ordinary model responses.

High-confidence current-information requests are a deliberate truth boundary: when `assess_current_information` detects freshness signals (today/latest/recent/updates/happening/current officeholder/price/status/etc.), the orchestrator forces a single authorized `web.current.search` execution before synthesis, injects the live receipt, and replaces false no-live-access denials with a bounded fallback built from that receipt. Ordinary non-fresh questions still rely on model-selected tools.
...
The application shell is viewport-bounded. The message region alone scrolls; the avatar and composer remain fixed. Voice dictation uses the browser SpeechRecognition path with one session per click, exposes listening/cancel/error states, writes interim/final transcription into the ordinary composer, and never restarts itself from `onend`.
