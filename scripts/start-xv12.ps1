...
try {
    ...
    try {
        ...
        Write-XV12Log "OPTIONAL MEDIA WARNING: ComfyUI startup failed: $($_.Exception.Message)"
    }
    ...
    if ($script:RuntimeConfig.media.image.enabled -and -not $health.services.creator.image_provider_status.healthy) {
        Write-XV12Log 'OPTIONAL MEDIA WARNING: XODUZ core is ready, but the configured ComfyUI image provider is unavailable.'
    }
