param(
    [ValidateSet('all','x-core','chat-core','auth','authorization','permissions','admin-capabilities','session','memory-isolation','context','model-runtime','launcher','registry-gateway','registry','gateway','ui-shell','user-identity','voice','voice-output','stt','tts','voice-settings','project-context','capability-registry','web','files','adas','calibration-iq','standalone','databases','attachments')]
    [string]$Pack = 'all'
)
...
$markers = @{
    ...
    files='files'; adas='adas'; 'calibration-iq'='calibration_iq'; standalone='standalone'; stt='stt or voice'; tts='tts or voice_output'; 'voice-settings'='voice_settings or voice_output'
}
$arguments = @('-m','pytest')
